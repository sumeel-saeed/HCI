...
<body>
    ...
    <table>
        <tr>
            <th width="150px">Report ID<br><hr></th>
            <th width="250px">Report Name<br><hr></th>
            <th width="150px"> Total Sale<br><hr></th>
            <th width="150px">Inventory Value<br><hr></th>
        </tr>
        <?php
        $sql = mysqli_query($link, "SELECT report_id, reportname, totalsale, inventoryvalue FROM `report`");
        while ($row = $sql->fetch_assoc()) {
            echo "<tr>
                <td>{$row['report_id']}</td>
                <td>{$row['reportname']}</td>
                <td>{$row['totalsale']}</td>
                <td>{$row['inventoryvalue']}</td>
            </tr>";
        }
        ?>
    </table>
